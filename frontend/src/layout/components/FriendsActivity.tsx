		...
		<div className='h-full bg-white/80 dark:bg-zinc-900 border border-amber-200 dark:border-zinc-800 rounded-lg flex flex-col'>
			...
			<div className='p-4 flex justify-between items-center border-b border-amber-200 dark:border-zinc-800'>
				<div className='flex items-center gap-2'>
					...
					<h2 className='font-semibold text-amber-900 dark:text-white'>Những Phật Tử đang nghe pháp</h2>
				</div>
			</div>
					...
					<div className='p-4 space-y-4'>
						{users.map((user) => {
							...
							return (
								<div
									key={user._id}
									className='cursor-pointer hover:bg-amber-100 dark:hover:bg-zinc-800/50 p-3 rounded-md transition-colors group'
								>
									<div className='flex items-start gap-3'>
										<div className='relative'>
											<Avatar className='size-10 border border-amber-200 dark:border-zinc-800'>
										...
										<div className='flex-1 min-w-0'>
											<div className='flex items-center gap-2'>
												<span className='font-medium text-sm text-amber-900 dark:text-white'>{user.fullName}</span>
												{user.isAdmin && (
											...
											{isPlaying ? (
												<div className='mt-1'>
													<div className='mt-1 text-sm text-amber-900 dark:text-white font-medium truncate'>
														{songName}
													</div>
													<div className='text-xs text-amber-700 dark:text-zinc-400 truncate'>
														{artistName}
													</div>
												</div>
											) : (
												<div className='mt-1 text-xs text-amber-700 dark:text-zinc-400'>Đang niệm A Di Đà Phật</div>
											)}
										</div>
									</div>
								</div>
	...
	<div className='h-full flex flex-col items-center justify-center p-6 text-center space-y-4'>
       ...
       opacity-75 animate-pulse'
		...
		<div className='space-y-2 max-w-[250px]'>
			<h3 className='text-lg font-semibold text-amber-900 dark:text-white'>Xem Phật Tử Khác Đang Nghe Gì</h3>
			<p className='text-sm text-amber-700 dark:text-zinc-400'>Đăng nhập để biết các Phật tử khác đang thưởng thức bài pháp nào</p>
		</div>
	</div>
);
